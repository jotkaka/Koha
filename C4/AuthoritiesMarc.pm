...
sub AddAuthority {
    ...
    if ($stored_control_number) {
        ...
        if ( !$record_control_number ) {
            ...
            $record->delete_fields( $record->field('010') );    # just in case strange 010 existed
            $record->insert_fields_ordered( MARC::Field->new( '010', ' ', ' ', a => $stored_control_number ) );
        }
    } else {
        my $prefix =
              ( $record->field('1..')->as_string('xyzv') || $record->field('1..')->tag =~ /^15/ ) ? 's'
            : $record->field('130')                                                               ? 't'
        ...
        if ( $action eq 'create' ) {
            if ( C4::Context->interface eq 'intranet' ) {
                ...
                $record->insert_fields_ordered( MARC::Field->new( '010', ' ', ' ', a => $new_control_number ) );
