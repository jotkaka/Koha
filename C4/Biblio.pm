...
sub _koha_delete_biblio_metadata {
    ...
    my $deleted_biblio_metadata =
        $schema->resultset('DeletedbiblioMetadata')->find( { biblionumber => $biblionumber } );
    my $deleted_biblio_record = MARC::Record::new_from_xml(
        $deleted_biblio_metadata->metadata,
        'UTF-8', 'MARC21'
    );
    ...
    my $sth =
        $dbh->prepare('SELECT branchcode FROM koha_plugin_pl_edu_nukat_symbole_nukat_symbols WHERE biblionumber = ?');
    $sth->execute($biblionumber);
    my @new_850;
    ...
    $deleted_biblio_metadata->update( { metadata => $deleted_biblio_record->as_xml_record('MARC21') } );
}
